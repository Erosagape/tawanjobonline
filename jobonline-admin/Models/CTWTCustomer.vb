...
Public Class TWTCustomer
    ...
    Public Sub AddNew()
        ...
        m_CustMessage = ""
    End Sub
    Public Function GetData(pSQLWhere As String) As List(Of TWTCustomer)
        Dim lst As New List(Of TWTCustomer)
        Using cn As New SqlConnection(m_ConnStr)
            Dim row As TWTCustomer
            Try
                cn.Open()
                Dim rd As SqlDataReader = New SqlCommand("SELECT * FROM TWTCustomer" & pSQLWhere, cn).ExecuteReader()
                While rd.Read()
                    row = New TWTCustomer(m_ConnStr)
                    ...
                    If IsDBNull(rd.GetValue(rd.GetOrdinal("Active"))) = False Then
                        row.Active = rd.GetBoolean(rd.GetOrdinal("Active")).ToString()
                    End If
