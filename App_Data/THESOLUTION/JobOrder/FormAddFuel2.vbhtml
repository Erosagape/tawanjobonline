...
<script type="text/javascript">
    ...
    function LoadJob(branch, job) {
        $.get(path + 'JobOrder/getjobosql?Branch=' + branch + '&Job=' + job).done(function (r) {
            if (r.job.data.length > 0) {
                ...
                $('#getContainerDate').text(ShowDate(rec.FactoryTime));
                $('#getContainerTime').text(ShowTime(rec.FactoryTime));
                $('#loadPlace').text(rec.PackingAddress);
                $('#loadDate').text(ShowDate(rec.PackingTime));
                $('#loadTime').text(ShowTime(rec.PackingTime));
                $('#returnContainerPlace').text(rec.ReturnAddress);
                $('#returnContainerDate').text(ShowDate(rec.ReturnTime));
                $('#returnContainerTime').text(ShowTime(rec.PackingTime));
