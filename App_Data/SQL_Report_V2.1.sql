...
Sum(CASE WHEN id.AmtAdvance >0 AND ISNULL(rh.CancelProve,'''')='''' THEN rd.Net ELSE 0 END) as TotalAdvance,
...
Sum(CASE WHEN ISNULL(rh.CancelProve,'''')='''' THEN rd.AmtVat ELSE 0 END) as TotalVat,
Sum(CASE WHEN ISNULL(rh.CancelProve,'''')='''' THEN rd.Amt50Tavi ELSE 0 END) as TotalWhtax,
...
AND a.DocNo=c.DocNo 
WHERE a.PRType=''P'' AND (a.CashAmount+a.ChqAmount)>0 AND NOT ISNULL(b.CancelProve,'''')<>''''
...
ON a.BranchCode=c.BranchCode
AND a.DocNo=c.DocNo 
WHERE a.PRType=''R'' AND (a.CashAmount+a.ChqAmount)>0 AND NOT ISNULL(b.CancelProve,'''')<>''''
) t {0} ORDER BY VoucherDate,ControlNo')
GO
...
'''' as ''Unit Code'',
'''' as ''Unit Price'',
h.TotalNet+h.Total50Tavi as ''Extend Amt'',
'''' as ''Account'',
'''' as ''Cost Center'',
'''' as ''PSL'',
'''' as TaxCode,
...
INNER JOIN Job_SrvSingle s ON d.SICode=s.SICode
WHERE d.AmtVat>0 AND ISNULL(h.CancelProve,'''')=''''  {0}
GROUP BY c.LnNO,c.GLAccountCode,h.DocNo,h.CurrencyCode
) t
ORDER BY 5')
GO
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_TAXDAILY', N'MAIN_SQL', N'SELECT rc.ReceiptNo,rc.ReceiptDate,rc.CustCode,rc.InvoiceNo,rc.Net as TotalReceived,rc.PRVoucher FROM (
SELECT rh.*,c1.UsedLanguage,
c1.Title + '' ''+ c1.NameThai as CustTName,c1.NameEng as CustEName,c1.TAddress1+'' ''+c1.TAddress2 as CustTAddr,c1.EAddress1+'' ''+c1.EAddress2 as CustEAddr,c1.Phone as CustPhone,c1.TaxNumber as CustTaxID,
c2.Title + '' ''+ c2.NameThai as BillTName,c2.NameEng as BillEName,c2.TAddress1+'' ''+c2.TAddress2 as BillTAddr,c2.EAddress1+'' ''+c2.EAddress2 as BillEAddr,c2.Phone as BillPhone,c2.TaxNumber as BillTaxID,
rd.ItemNo,rd.InvoiceNo,rd.InvoiceItemNo,ih.DocDate as InvoiceDate,ih.BillAcceptNo,ih.BillIssueDate,ih.BillAcceptDate,ih.RefNo,
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_TAXSUMMARY', N'MAIN_SQL', N'SELECT rc.ReceiptNo,rc.ReceiptDate,rc.CustCode,rc.CustTName as NameThai,rc.InvoiceNo,rc.JobNo,SUM(CASE WHEN rc.AmtAdvance>0 THEN rc.Amt ELSE 0 END) as AmtAdvance,
SUM(CASE WHEN rc.AmtCharge>0 THEN rc.Amt ELSE 0 END) as TotalCharge,
SUM(rc.AmtVAT) as AmtVAT,SUM(rc.Amt50Tavi) as Amt50Tavi,SUM(rc.Net) as AmtNet,SUM(CASE WHEN ISNULL(rc.PRVoucher,'''')<>'''' THEN 0 ELSE rc.Net END) as TotalReceived FROM (
SELECT rh.*,c1.UsedLanguage,
c1.Title + '' ''+ c1.NameThai as CustTName,c1.NameEng as CustEName,c1.TAddress1+'' ''+c1.TAddress2 as CustTAddr,c1.EAddress1+'' ''+c1.EAddress2 as CustEAddr,c1.Phone as CustPhone,c1.TaxNumber as CustTaxID,
c2.Title + '' ''+ c2.NameThai as BillTName,c2.NameEng as BillEName,c2.TAddress1+'' ''+c2.TAddress2 as BillTAddr,c2.EAddress1+'' ''+c2.EAddress2 as BillEAddr,c2.Phone as BillPhone,c2.TaxNumber as BillTaxID,
rd.ItemNo,rd.InvoiceNo,rd.InvoiceItemNo,ih.DocDate as InvoiceDate,ih.BillAcceptNo,ih.BillIssueDate,ih.BillAcceptDate,ih.RefNo,
...
INNER JOIN Job_InvoiceHeader ih ON rd.BranchCode=ih.BranchCode AND rd.InvoiceNo=ih.DocNo 
LEFT JOIN Job_CashControlSub vd ON rd.BranchCode=vd.BranchCode AND rd.ControlNo=vd.ControlNo AND rd.ControlItemNo=vd.ItemNo
 WHERE rh.TotalVAT>0 AND NOT ISNULL(rh.CancelProve,'''')<>'''' AND NOT ISNULL(ih.CancelProve,'''')<>'''' {0}) rc GROUP BY rc.ReceiptNo,rc.ReceiptDate,rc.CustCode,rc.CustTName,rc.InvoiceNo,rc.JobNo ORDER BY rc.CustTName,rc.ReceiptDate,rc.ReceiptNo')
GO
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_TRUCKPNL', N'MAIN_CLITERIA', N'AND,d.UnloadFinishDate,j.CustCode,j.JNo,j.CSCode,j.AgentCode,j.JobStatus,j.BranchCode')
GO
...
d.UnloadFinishDate as DeliveryDate,d.ReturnDate,h.VenderCode,a.AccTName,d.CTN_NO,
(SELECT STUFF((
...
CONVERT(numeric(10,2),e.TotalExpense) as TotalCost,e.TotalInvoice as ''Invoice Billed'',e.TotalInvoice-e.TotalExpense as ''Profit'',
...
LEFT JOIN (
	...
	SUM(pd.Amt) as TotalExpense,SUM(clr.TotalInv) as TotalInvoice
	FROM Job_PaymentHeader ph INNER JOIN Job_PaymentDetail pd
	ON ph.BranchCode=pd.BranchCode AND ph.DocNo=pd.DocNo
	LEFT JOIN
	(
		SELECT BranchCode,DocNo,ItemNo,SUM(UsedAmount) as TotalInv FROM (
		select ch.BranchCode,SUBSTRING(cd.VenderBillingNo,1,CHARINDEX(''#'',cd.VenderBillingNo)-1) as DocNo,
		SUBSTRING(cd.VenderBillingNo,CHARINDEX(''#'',cd.VenderBillingNo)+1,2) as ItemNo,
		cd.UsedAmount,cd.SDescription
		from Job_ClearDetail cd inner join Job_ClearHeader ch
		on cd.BranchCode=ch.BranchCode AND cd.ClrNo=ch.ClrNo
		where CHARINDEX(''#'',cd.VenderBillingNo)>0
		AND ch.DocStatus<>99
		AND SUBSTRING(cd.VenderBillingNo,CHARINDEX(''#'',cd.VenderBillingNo)+1,2)>0 
		AND LinkItem>0
		union
		select ch.BranchCode,SUBSTRING(cd.VenderBillingNo,1,CHARINDEX(''#'',cd.VenderBillingNo)-1) as DocNo,
		1 as ItemNo,
		...
		AND SUBSTRING(cd.VenderBillingNo,CHARINDEX(''#'',cd.VenderBillingNo)+1,2)=0 
		AND LinkItem>0
		) cl GROUP BY BranchCode,DocNo,ItemNo ) clr
	ON pd.BranchCode=clr.BranchCode AND pd.DocNo=clr.DocNo AND pd.ItemNo=clr.ItemNo
	WHERE NOT ISNULL(ph.CancelProve,'''')<>'''' {0}
...
FROM Job_ReceiptHeader h inner join (
	...
	SUM(CASE WHEN b.AmtCharge>0 AND b.AmtVat>0 THEN a.Amt ELSE 0 END) as TotalVATCharge,
	SUM(CASE WHEN b.AmtCharge>0 AND b.AmtVat=0 THEN a.Amt ELSE 0 END) as TotalNonVAT
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_ACCEXP', N'MAIN_SQL', N'SELECT j.CustCode,c.ClrNo,c.ClrDate,c.JobNo,c.SDescription,c.SlipNo,c.VenderCode,
c.AmtAdvance,c.AmtCostSlip,c.AmtCostNoSlip,c.InvoiceNo,c.AmtCostUnClear as AmountRemain,c.RefNo,c.PaymentDate FROM (
select h.BranchCode,h.ClrNo,h.ClrDate,d.JobNo,d.SDescription,d.SlipNo,d.VenderCode,d.UsedAmount,d.BNet,
(CASE WHEN s.IsCredit=1 AND s.IsExpense=0 THEN d.BNet ELSE 0 END) as AmtAdvance,
(CASE WHEN s.IsHaveSlip=1 AND s.IsExpense=1 THEN d.BNet ELSE 0 END) as AmtCostSlip,
(CASE WHEN s.IsHaveSlip=0 AND s.IsExpense=1 THEN d.BNet ELSE 0 END) as AmtCostNoSlip,
d.LinkBillNo as InvoiceNo,
(CASE WHEN ISNULL(d.LinkBillNo,'''')='''' AND s.IsExpense=0 AND s.IsCredit=1 THEN d.BNet ELSE 0 END) as AmtCostUnClear,
a.AdvNo as RefNo,a.PaymentDate
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_ACCINC', N'MAIN_SQL', N'SELECT j.CustCode,c.ClrNo,c.ClrDate,c.JobNo,c.SDescription,c.SlipNo,c.VenderCode,c.AmtCharge as ServiceAmount,
c.AmtVat,c.AmtWht as TotalWHT,c.InvoiceNo,c.AmtChargeUnBill as AmountRemain
 FROM (
select h.BranchCode,h.ClrNo,h.ClrDate,d.JobNo,d.SDescription,d.SlipNo,d.VenderCode,
(CASE WHEN s.IsCredit=0 AND s.IsExpense=0 THEN d.UsedAmount ELSE 0 END) as AmtCharge,
...
d.LinkBillNo as InvoiceNo,
(CASE WHEN ISNULL(d.LinkBillNo,'''')='''' THEN d.UsedAmount ELSE 0 END) as AmtChargeUnBill
from Job_ClearDetail d inner join Job_ClearHeader h
ON d.BranchCode=h.BranchCode AND d.ClrNo=h.ClrNo
inner join Job_AdvHeader a ON d.BranchCode=a.BranchCode AND d.AdvNO=a.AdvNo
left join Job_SrvSingle s ON d.SICode=s.SICode
...
ad.AdvPayAmount as TotalAdvance,ad.Charge50Tavi as TotalWHT FROM (
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_ADVDETAIL', N'MAIN_CLITERIA', N'WHERE,j.DocDate,a.CustCode,d.ForJNo,a.ReqBy,d.VenCode,a.DocStatus,a.BranchCode,d.SICode')
GO
...
d.SDescription,d.AdvAmount+d.ChargeVAT as PaidAmount,p.ChqDate,p.ChqNo
FROM Job_AdvHeader as a LEFT JOIN
Mas_Company b ON a.CustCode=b.CustCode AND a.CustBranch=b.Branch
LEFT JOIN (
    SELECT *,
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_CASHBAL', N'MAIN_SQL', N'SELECT acType,PRType,PRVoucher,VoucherDate,TRemark,ChqNo,ChqDate,(CASE WHEN PRType=''P'' THEN (CashAmount+ChqAmount)*-1 ELSE (CashAmount+ChqAmount) END) as TotalNet,ControlNo FROM (
SELECT h.BranchCode, h.ControlNo, h.VoucherDate, h.TRemark, h.RecUser, h.RecDate, h.RecTime, h.PostedBy, h.PostedDate, h.PostedTime, 
    h.CancelReson, h.CancelProve, h.CancelDate, h.CancelTime, h.CustCode, h.CustBranch, d.ItemNo, d.PRVoucher, d.PRType, d.ChqNo, d.BookCode, d.BankCode, 
    d.BankBranch, d.ChqDate, d.CashAmount, d.ChqAmount, d.CreditAmount, d.IsLocal, d.ChqStatus, d.PayChqTo, d.SICode, d.RecvBank, d.RecvBranch, d.acType, 
    d.SumAmount, d.CurrencyCode, d.ExchangeRate, d.TotalAmount, d.VatInc, d.VatExc, d.WhtInc, d.WhtExc, d.TotalNet, d.ForJNo
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_CASHFLOW', N'MAIN_SQL', N'SELECT PRType,PRVoucher,VoucherDate,TRemark,ChqNo,ChqDate,(CASE WHEN PRType=''P'' THEN (CashAmount+ChqAmount+CreditAmount) ELSE 0 END) as PaidAmount,
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_CLRDAILY', N'MAIN_SQL', N'SELECT cl.ClrNo,cl.ClrDate,cl.SDescription,cl.AdvNO,cl.JobNo,cl.AdvAmount as AdvNet,cl.ClrNet,cl.Tax50Tavi as TotalWHT,cl.SlipNo,cl.LinkBillNo as InvoiceNo FROM (select 
...
a.BPrice as SumCharge,a.ChargeVAT as AmtVat,a.Tax50Tavi as Amt50Tavi,(CASE WHEN ISNULL(a.LinkBillNo,'''')<>'''' THEN a.BNet ELSE 0 END) as ''Invoice Billed'',
(CASE WHEN ISNULL(a.LinkBillNo,'''')<>'''' THEN 0 ELSE a.BNet END) as TotalBalance,a.LinkBillNo
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_CLRSUMMARY', N'MAIN_SQL', N'SELECT cl.ClrNo,cl.ClrDate,cl.JobNo,cl.DutyDate,SUM(cl.UsedAmount) as UsedAmount,SUM(cl.ChargeVAT) as AmtVat,SUM(cl.Tax50Tavi) as TotalWHT,SUM(CASE WHEN cl.IsExpense=0 AND cl.IsCredit=1 THEN cl.UsedAmount ELSE 0 END) as SumAdvance,SUM(CASE WHEN cl.IsExpense=1 THEN cl.UsedAmount ELSE 0 END) as SumCost,SUM(CASE WHEN cl.IsExpense=0 AND cl.IsCredit=0 THEN cl.UsedAmount ELSE 0 END) as SumCharge,cl.LinkBillNo as InvoiceNo FROM (select 
h.BranchCode,h.ClrNo,h.ClrDate,h.DocStatus,c1.ClrStatusName,
h.ClearanceDate,h.JobType,c4.JobTypeName,j.ShipBy,c6.ShipByName,
b.BrName as BranchName,h.CTN_NO,h.CoPersonCode,h.TRemark,h.ClearType,c2.ClrTypeName,h.ClearFrom,c3.ClrFromName,
h.EmpCode,u1.TName as ClrByName,h.ApproveBy,u2.TName as ApproveByName,
...
SUM(AdvBilled) as TotalAdv,SUM(ChargeCleared) as TotalCharge,
...
(CASE WHEN d.IsExpense=1 AND NOT d.GroupCode=''ERN''  THEN a.UsedAmount ELSE 0 END) as CostCleared,
...
(CASE WHEN t.CashAmount<>'''' THEN ''<br/>CASH Ref# '' + t.CashRef ELSE '''' END),
(CASE WHEN t.ChqAmount<>'''' THEN ''<br/>CHEQUE Ref# '' + t.ChqRef ELSE '''' END),
''</span><span style="flex:1">'',
(CASE WHEN t.CashAmount<>'''' THEN ''<br/>Bank# '' + t.CashBank ELSE '''' END),
(CASE WHEN t.ChqAmount<>'''' THEN ''<br/>Bank# '' + t.ChqBank ELSE '''' END),
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_INVDAILY', N'MAIN_SQL', N'SELECT inv.DocNo,inv.DocDate,inv.BillIssueDate,inv.BillAcceptNo,inv.BillAcceptDate,inv.DueDate,inv.SDescription,inv.Amt,inv.AmtVat,inv.AmtCredit,inv.Amt50Tavi,inv.TotalInv as TotalNet,inv.CreditNet as ''Adjust Amount'',inv.ReceivedNet as TotalReceived,inv.ReceiptNo,inv.LastVoucher FROM (
...
GO
INSERT [dbo].[Mas_Config] ([ConfigCode], [ConfigKey], [ConfigValue]) VALUES (N'REPORT_INVDETAIL', N'MAIN_SQL', N'SELECT inv.DocNo,inv.DocDate,inv.RefNo,inv.SDescription,inv.AmtAdvance,inv.AmtCharge as TotalCharge,inv.AmtCredit,inv.AmtVat,inv.Amt50Tavi,inv.TotalInv as TotalNet,inv.ReceivedNet as TotalReceived,inv.ReceiptNo,inv.LastVoucher FROM (
select ih.*,id.ItemNo,id.SICode,id.SDescription,id.ExpSlipNO,id.SRemark,
id.Amt,id.AmtDiscount,id.AmtCredit,
id.AmtCharge,id.AmtAdvance,id.AmtVat,id.Amt50Tavi,
id.TotalAmt,id.TotalAmt-id.AmtCredit as TotalInv,
...
sum(CASE WHEN inv.CancelProve<>'''' THEN 0 ELSE inv.AmtVat END) as TotalVAT,
sum(CASE WHEN inv.CancelProve<>'''' THEN 0 ELSE inv.Amt50Tavi END) as Total50Tavi,
...
SUM(CASE WHEN sv.IsExpense=1 AND cd.LinkItem=0 THEN cd.UsedAmount ELSE 0 END) AS SumCost,
...
SUM(CASE WHEN sv.IsCredit=0 AND sv.IsExpense=0 THEN cd.UsedAmount ELSE 0 END)-SUM(CASE WHEN sv.IsExpense=1 AND cd.LinkItem=0 THEN cd.UsedAmount ELSE 0 END) as Profit,
j.CSCode,j.ShippingEmp,ISNULL(ad.AdvNet,0) as TotalAdvance,
SUM(CASE WHEN cd.AdvAmount>0 THEN cd.UsedAmount ELSE 0 END) AS TotalExpClear,
(CASE WHEN ISNULL(ad.AdvNet,0)>0 AND ISNULL(ad.AdvNet,0)>=SUM(CASE WHEN cd.AdvAmount>0 THEN cd.UsedAmount ELSE 0 END) THEN ISNULL(ad.AdvNet,0)-SUM(CASE WHEN cd.AdvAmount>0 THEN cd.UsedAmount ELSE 0 END) ELSE 0 END) as TotalReturn,
(CASE WHEN ISNULL(ad.AdvNet,0)>0 AND ISNULL(ad.AdvNet,0)>=SUM(CASE WHEN cd.AdvAmount>0 THEN cd.UsedAmount ELSE 0 END) THEN 0 ELSE SUM(CASE WHEN cd.AdvAmount>0 THEN cd.UsedAmount ELSE 0 END)-ISNULL(ad.AdvNet,0) END) as TotalPayback,
SUM(CASE WHEN CHARINDEX(''ERN'',cd.SICode)>0 THEN cd.UsedAmount ELSE 0 END) AS TotalEarnest,
(SELECT TOP 1 VoucherDate FROM Job_CashControl WHERE BranchCode=j.BranchCode AND ControlNo= MAX(CASE WHEN CHARINDEX(''ERN'',cd.SICode)>0 And cd.LinkBillNo<>'''' THEN cd.LinkBillNo ELSE '''' END) ) AS DepositReturnDate,
j.CloseJobDate
FROM dbo.Job_Order AS j
 LEFT JOIN dbo.Job_ClearDetail AS cd ON j.BranchCode = cd.BranchCode AND j.JNo = cd.JobNo
...
LEFT JOIN Mas_Vender v ON j.ForwarderCode=v.VenCode
LEFT JOIN (SELECT b.BranchCode,b.ForJNo,SUM(b.AdvNet) as AdvNet FROM Job_AdvHeader a INNER JOIN Job_AdvDetail b
ON a.BranchCode=b.BranchCode AND a.AdvNo=b.AdvNo WHERE a.DocStatus<>99 AND b.ForJNo<>'''' GROUP BY b.BranchCode,b.ForJNo) ad
ON j.BranchCode=ad.BranchCode AND j.JNo=ad.ForJNo
WHERE (j.JobStatus < 90) AND (isnull(ch.DocStatus,0)<>99) {0}
GROUP BY j.BranchCode,j.DocDate,j.DutyDate,j.JNo,jt.JobTypeName,sb.ShipByName
,c.NameEng,e.NameEng,j.DeliveryTo,v.TName,j.InvProduct,j.InvNo,j.HAWB,j.DeclareNumber,j.ETDDate,j.ETADate,j.LoadDate,j.EstDeliverDate,j.TotalContainer,j.CSCode,j.ShippingEmp
,ad.AdvNet,j.CloseJobDate
...
SUM(CASE WHEN sv.IsCredit=1 AND sv.IsExpense=0 THEN cd.BNet ELSE 0 END) AS SumAdvance,
SUM(CASE WHEN sv.IsCredit=0 AND sv.IsExpense=0 THEN cd.UsedAmount ELSE 0 END) AS SumCharge,
SUM(CASE WHEN sv.GroupCode=''ERN'' THEN cd.UsedAmount ELSE 0 END) AS TotalEarnest,
...
0 as ''CN,DN Amount'',id.AmtAdvance as ''Advance Amount'',id.AmtCharge as ''SVC Amount'',ISNULL(rd.ReceiptNet,0) as ''Payment Received'',
...
left join (
	...
	where NOT ISNULL(h.CancelProve,'''')<>'''' AND d.VoucherNo<>''''
	group by d.BranchCode,d.InvoiceNo,d.InvoiceItemNo
