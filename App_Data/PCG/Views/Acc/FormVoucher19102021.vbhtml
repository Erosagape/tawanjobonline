    ...
    function ShowData(data) {
        ...
        if (data.payment !== null) {
            for(let obj of data.payment) {
                ...
                let desc = '';
                let desc0 = '';
		let totalamt=Number(obj.CashAmount)+Number(obj.ChqAmount);
		console.log(Number(obj.CashAmount)+Number(obj.ChqAmount));
		if(Number(obj.TotalAmount)>0) totalamt=Number(obj.TotalAmount);
		console.log(Number(obj.TotalAmount));

               // appendLine(div, '<b>' + vcTypeName + ' BY ' + acTypeName + '</b>',obj.PRVoucher,CCurrency(CDbl(totalamt + Number(CDbl(obj.VatExc,2)),2)));
  		appendLine(div, '<b>' + vcTypeName + ' BY ' + acTypeName + '</b>',obj.PRVoucher,CCurrency(CDbl(totalamt + Number(CDbl(obj.WhtExc,2)) -  Number(CDbl(obj.WhtInc,2)),2)));
                ...
                desc2 += CCurrency(CDbl(totalamt+Number(obj.WhtExc)-Number(obj.WhtInc)+Number(obj.VatInc)-Number(obj.VatExc),2));
                desc2 += '</td>';
                desc2 += '</tr>';
                desc2 += '<tr>';
                desc2 += '<td width="20%" style="text-align:right">';
                desc2 += CCurrency(CDbl(Number(obj.VatExc)+Number(obj.VatInc),2));
                desc2 += '</td>';
                desc2 += '</tr>';
                desc2 += '<tr>';
                desc2 += '<td width="20%" style="text-align:right">';
                desc2 += CCurrency(CDbl(Number(obj.WhtExc)+Number(obj.WhtInc),2));
                desc2 += '</td>';
                desc2 += '</tr>';
                desc2 += '<tr>';
                desc2 += '<td width="20%" style="text-align:right">';
