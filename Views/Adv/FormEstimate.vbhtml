...
<div id="address">
    <div>
        <b>SHIPPER : </b><label id="lblCustName"></label>
        <br />
        <b>ADDRESS: </b><label id="lblCustAddress1"></label>
        <label id="lblCustAddress2"></label>
    </div>
    <div style="width:1em"></div>
    <div>
        <b>CONSIGNEE : </b><label id="lblConsingeeName"></label>
        <br />
        <b>ADDRESS: </b><label id="lblConsingeeAddress1"></label>
        <label id="lblConsingeeAddress2"></label>
    </div>
</div>
...
<table id="dvHeader" style="width:100%">
    <thead d="dvHeader">
        <tr style="border-top: 1px solid black; border-bottom: 1px solid black; background-color:#66FFCC">
            ...
            <th class="vborder" style="width:5%">UNITP</th>
            <th class="vborder" style="width:5%">USD</th>
            ...
            <th class="vborder" style="width:10%">NO VAT</th>
    ...
    <tbody id="dvHeader2">
        <tr style="border-top: 1px solid black; border-bottom: 1px solid black; background-color:lightgoldenrodyellow">
            <th class="vborder" style="width:10%">DESCRIPTION</th>
            <th class="vborder" style="width:5%">EXC</th>
            <th class="vborder" style="width:5%">CUR</th>
            <th class="vborder" style="width:5%">QTY</th>
...
<div></div>
<p>
    <center><label style="font-size:10px;">รบกวนลูกค้าโอนชำระเงินตามเลขบัญชีด้านล่างเท่านั้น พร้อมส่งหลักฐาน การโอนเงินและสำเนาใบหักมาด้วยทุกครั้ง ล่วงหน้าอย่างน้อย 2 วัน ก่อนเข้ามารับใบเสร็จค่า D/O  </label></center>
    <br />
    <center>บริษัทฯ ขอสงวนสิทธิ์ไม่รับเงินสดและเช็คในทุกกรณี &nbsp; <label style="font-size:0.8em;font-weight:bold">BANK TRANSFER ONLY. CASH AND CHEQUE NOT ACCEPTED.</label></center>
    <br />
    <br />
    ...
    218-301-7405<br />
    BANGKOK BANK<br />
    “ BETTER FREIGHT AND TRANSPORT CO.,LTD.”<br /><br />
...
<script type="text/javascript">
    ...
    function ReadJob(dt) {
        ...
        ShowCountry(path, dr.InvFCountry, '#lblFromCountry');
        ShowCountry(path, dr.InvCountry, '#lblToCountry');

        $("#lblToPort").text(dr.ClearPortNo);
        ShowInterPort(path, (dr.JobType == 1 ? dr.InvFCountry : dr.InvCountry), dr.InvInterPort, '#lblFromPort').then(() => {
            ...
            $.get(path + 'master/getcompany?Code=' + dr.Consigneecode).done(function (r) {
                if (r.company.data.length > 0) {
                    ...
                    $('#lblConsingeeName').text(data.NameEng);
                    $('#lblConsingeeAddress1').text(data.EAddress1);
                    $('#lblConsingeeAddress2').text(data.EAddress2);
                }
            });
    ...
    function ReadData(dt) {
        ...
        for (const propname in incomegroup) {
            ...
            html += '<tr><td   style="border:1px black solid;font-weight:bold" colspan="13"><pre style="padding:0;margin:0;white-space:pre-wrap">' + propname + '</pre></td></tr>';
            for (let r of rows) {
                ...
                html += '<td class="vborder" style="width:5%;text-align:right">' + (r.IsCredit == 0 && r.IsExpense == 0 ? CCurrency(CDbl(r.AmountCharge, 2)) : "") + '</td>';
                html += '<td class="vborder" style="width:5%;text-align:right">' + (r.IsCredit == 0 && r.IsExpense == 0 && r.ExchangeRate > 1? CCurrency(CDbl(r.AmountCharge * r.Qty, 2)) : "") + '</td>';
                ...
                if (r.IsCredit == 0 && r.IsExpense == 0) {
                    ...
                    if (r.ExchangeRate > 1) {
                        groupFAmount += (r.AmountCharge * r.Qty) - 0;
                        totalFAmtGet += (r.AmountCharge * r.Qty) - 0;
                    }
            ...
            groupChargeHtml += '<td class="vborder" style="text-align:center"></td>';
            groupChargeHtml += '<td class="vborder" style="text-align:center"></td>';
            groupChargeHtml += '<td class="vborder" style="text-align:right">' + CCurrency(CDbl(groupFAmount, 2)) + '</td>';
        ...
        let sumCharge = "";
        sumCharge += '<tr  style="border-top:1px solid black;border-bottom:1px solid black;background-color:#66FFCC">';
        ...
        sumCharge += '<td class="vborder" style="text-align:center"></td>';
        sumCharge += '<td class="vborder" style="text-align:center"></td>';
        sumCharge += '<td class="vborder" style="text-align:right">' + CCurrency(CDbl(totalFAmtGet, 2)) + '</td>';
        ...
        sumCharge += '<td class="vborder" style="font-weight:bold">ยอดที่ต้องชำระ</td>';
        sumCharge += '<td class="vborder" style="text-align:center;font-weight:bold" colspan="9">' + CNumThai(CDbl(totalCharge, 2)) + '</td>';
        sumCharge += '<td class="vborder" colspan="3" style="text-align:right;font-weight:bold">' + CCurrency(CDbl(totalCharge, 2)) + '</td>';
        sumCharge += '</tr>';
        ...
        for (const propname in expensegroup) {
            ...
            let groupAdvCost = 0;
            let groupFAmount = 0;
            html2 += '<tr><td   style="border:1px black solid;font-weight:bold" colspan="13"><pre style="padding:0;margin:0;white-space:pre-wrap">' + propname + '</pre></td></tr>';
            //html2 += '<tr><td   style="border:1px black solid;font-weight:bold" colspan="13">' + propname + '</td></tr>';
            for (let r of rows) {
                html2 += '<tr>';
                ...
                html2 += '<td class="vborder" style="width:5%;text-align:right">' + ((r.IsExpense == 1 && r.IsCredit == 0) || r.SICode.indexOf("INT") >= 0 ? CCurrency(CDbl(r.AmountCharge, 2)) : "") + '</td>';
                html2 += '<td class="vborder" style="width:5%;text-align:right">' + ((r.IsExpense == 1 && r.IsCredit == 0) || r.SICode.indexOf("INT") >= 0 ? CCurrency(CDbl(r.AmountCharge * r.Qty, 2)) : "") + '</td>';
                ...
                if ((r.IsExpense == 1 && r.IsCredit == 0) || r.SICode.indexOf("INT") >= 0) {
                    ...
                    if (r.ExchangeRate > 1) {
                        groupFAmount += (r.AmountCharge * r.Qty) - 0;
                        totalFAmtPay += (r.AmountCharge * r.Qty) - 0;
                    }
            ...
            groupCostHtml += '<td class="vborder" style="text-align:center"></td>';
            groupCostHtml += '<td class="vborder" style="text-align:center"></td>';
            groupCostHtml += '<td class="vborder" style="text-align:right">' + CCurrency(CDbl(groupFAmount, 2)) + '</td>';
        ...
        let sumCost = "";
        sumCost += '<tr  style="border-top:1px solid black;border-bottom:1px solid black;background-color:lightgoldenrodyellow">';
        ...
        sumCost += '<td class="vborder" style="text-align:center"></td>';
        sumCost += '<td class="vborder" style="text-align:center"></td>';
        sumCost += '<td class="vborder" style="text-align:right">' + CCurrency(CDbl(totalFAmtPay, 2)) + '</td>';
