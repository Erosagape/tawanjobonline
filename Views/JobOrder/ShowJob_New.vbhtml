                        ...
                        <div class="col-sm-8">
                            <div style="display:flex;flex-direction:column">
                                ...
                                <div>
                                    ...
                                    <input type="button" id="btnLinkJob" value="Search" class="btn btn-primary" onclick="SearchData('job')" />
                                </div>
                                <div>
    ...
    function SearchData(type) {
        switch (type) {
            ...
            case 'job':
                ...
                let url = 'http://'+location.hostname+'/ace/';
                if (lic.indexOf('โกลบอล') >= 0) {
                    url = 'http://'+location.hostname+'/acet/';
                }
        ...
        .done(function (r) {
            if (r.job.data.length > 0) {
                ...
                let tb=$('#tbTracking').DataTable({
			 	...
			 	render: function (data) {
                                return '['+data.DocStatus+'] '+data.StatusName;
                            }
...
*/
    ...
    function LoadPaperlessKSOFT() {
        ...
        $.get(path + 'JobOrder/GetPaperless' + url).done(function (r) {
            if (r.length > 0) {
                $('#txtCustInvNo').val(r[0].dcl_invno);
                ...
                $('#txtHAWB').val(r[0].housebl);
                $('#txtMAWB').val(r[0].bl);
                $('#txtInvPackQty').val(r[0].qty);
                $('#txtInvQty').val(r[0].nopkg);
                $('#txtInvUnit').val(r[0].pkgunit);
                $('#txtNetWeight').val(r[0].netww);
                $('#txtGrossWeight').val(r[0].grsww);
                $('#txtWeightUnit').val(r[0].grswwunit);
                $('#txtInvFCountryCode').val(r[0].im_consignmentcountry);
                $('#txtInvCountryCode').val(r[0].ex_destinationcountry);
                $('#txtEDIDate').val(CDateEN(r[0].ddtt));
                $('#txtETDDate').val(ExtractDate(r[0].departdd));
                $('#txtETADate').val(ExtractDate(r[0].imdate_submitdate));
                $('#txtDutyDate').val(ExtractDate(r[0].jobdate));
                ...
                ShowCountry(path, $('#txtInvCountryCode').val(), '#txtInvCountry');
                ShowCountry(path, $('#txtInvFCountryCode').val(), '#txtInvFCountry');

                ShowDeclareType(path, $('#txtDeclareType').val(), '#txtDeclareTypeName');
                ShowReleasePort(path, $('#txtReleasePort').val(), '#txtReleasePortName');

                ShowMessage('Please check updated data!');
            } else {
                ShowMessage('Data not found', true);
            }
        });
