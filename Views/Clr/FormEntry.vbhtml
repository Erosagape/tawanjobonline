    ...
    function getClearIngReport() {
        $.get(path + 'clr/getclearingreport?branch=' + branch + '&job=' + job, function (r) {
		    ...
		    let net = d[i].BNet;
                    ...
                    let customerSlip = (d[i].IsHaveSlip == 1 && d[i].IsCredit == 1 ? net : 0);
                    let companySlip = (d[i].IsHaveSlip == 1 && d[i].IsExpense == 1 ? net : 0);
                    let noSlip = d[i].IsHaveSlip == 0 && d[i].IsExpense == 1 ? net : 0;
                    let serv = (d[i].IsCredit == 0 && d[i].IsExpense == 0 ? net : 0);
                    let profit = (d[i].IsExpense == 1 ? amt * -1 : d[i].IsCredit == 1 ? 0 : net);
                    //amtadv += adv;
                    //amtserv += serv;
		...
		$("#sumService").text(CCurrency(CDbl(customerSlipSum + servSum, 2)));
                $("#sumCost").text(CCurrency(CDbl(customerSlipSum + companySlipSum +noSlipSum, 2)));
