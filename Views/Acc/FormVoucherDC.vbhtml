...
<script type="text/javascript">
    ...
    function ShowData(data) {
        ...
        for (pay of data.payment) {
            ...
            let dcGroup = groupBy(doc, 'LinkBillNo');
            console.log('----------');
            console.log(dcGroup);
            console.log('----------');


            for (const dcNo in dcGroup) {
                let tr = "";
                let cndn = dcGroup[dcNo];
                let fNet = 0;
                let bNet = 0;
                for (d of cndn) {
                    console.log(d);
                ...
                tr += `<tr style="height:0px">
                            <td style="text-align:center;padding:0px 5px;">${dcNo}</td>
                            <td style="text-align:center;padding:0px 5px;">${ShowDate(cndn[0].CDdate)}</td>
                            <td style="text-align:center;padding:0px 5px;">${cndn[0].JobNo}</td>
                            <td style="text-align:right;padding:0px 5px;">${cndn[0].NewBL ? cndn[0].NewBL : ""}</td>
                            <td style="text-align:right;padding:0px 5px;">${fNet >= 0 ? ShowNumber(Math.abs(fNet), 2) : "(" + ShowNumber(Math.abs(fNet), 2) + ")"}</td>
                            <td style="text-align:right;padding:0px 5px;">${cndn[0].ClrEXcRate}</td>
                            <td style="text-align:right;padding:0px 5px;">${bNet >= 0 ? ShowNumber(Math.abs(bNet), 2) : "(" + ShowNumber(Math.abs(bNet), 2) + ")"}</td>
                       </tr>`;
                detailTrs += tr;
            ...
            detailTrs  += `<tr style="height:0px;border-top:1px black solid">
                           <td  colspan="4" style="text-align:right;padding:0px 5px;font-weight:bold">TOTAL : USD </td>
                            <td style="text-align:right;padding:0px 5px;">${sumFNet >= 0 ? ShowNumber(sumFNet, 2) : "(" + ShowNumber(sumFNet, 2) + ")"}</td>
                            <td style="text-align:right;padding:0px 5px;"></td>
                            <td style="text-align:right;padding:0px 5px;">${sumBNet >= 0 ? ShowNumber(sumBNet, 2) : "(" + ShowNumber(sumBNet, 2) + ")"}</td>
                        </tr>`;
  //          if(totalNet>0){
		//totPay+=totalNet;
  //          } else {
                ...
                <thead>
                    <tr id="A1">
                        ...
                        <td style="width: 10%; ">
                         ...
                         <td  style="width:15%; ">
                            รหัสอ้างอิงงานหลัก<br/>
                            JOB REFERENCE
                        </td>
                        <td style="width:15%; ">
                            เลขที่ใบตราส่งย่อย<br/>
                            HOUSE B/L
                        </td>
                        <td style="width:15%; ">
                            จำนวนเงิน<br/>
