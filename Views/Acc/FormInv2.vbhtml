	...
	$("#decl").text(j.DeclareNumber);
 		...
 		//<td><label id="volumeLbl">VOLUME</label></td>
            ...
            for (let row of d) {
                ...
                html += '            <td class="right">' + ShowNumber(row.AmtAdvance ? row.FUnitPrice + (row.AmtVat / row.Qty) : row.FUnitPrice,3) + '</td>';
                ...
                html += '            <td class="right">' + (!row.AmtAdvance &&row.AmtVat>0?ShowNumber(row.Amt,3) : '') + '</td>';
                html += '        </tr>';
            ...
            $('#details').html(html);
            $("#valueAddedTax").text(ShowNumber(varFromSrv,3));
            $("#totalAmount").text(ShowNumber(adv+vat + h.TotalVAT,3));
            $("#lessWithholdingTax").text(ShowNumber(h.Total50Tavi,3));
            $("#netAmount").text(ShowNumber(adv + vat + nonVat + h.TotalVAT - h.Total50Tavi,3));
            $("#custAdv").text(ShowNumber(h.TotalCustAdv,3));
            $("#grandTotal").text(ShowNumber((adv + vat + nonVat + h.TotalVAT - h.Total50Tavi)-h.TotalCustAdv,3));
