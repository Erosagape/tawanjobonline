...
count(d.DocNo) as TotalDoc,SUM(d.PayAmount) as TotalPayAmount,sum(d.PayTax) as TotalPayTax,h.TaxLawNo,h.SeqInForm
...
from Job_WHTax h left join Job_WHTaxDetail d
on h.BranchCode=d.BranchCode and h.DocNo=d.DocNo
where  not isnull(h.CancelProve,'')<>'' AND h.FormType=" & frm & " AND Year(h.DocDate)={0} AND Month(h.DocDate)={1} AND h.TaxNumber1='{2}'
AND h.TaxNumber2='{3}'
group by h.TaxNumber3,Convert(numeric,'0'+h.Branch3),h.DocNo,
d.PayRate,d.PayDate,d.PayTaxDesc,h.PayTaxType
"
        End If
        ...
        For Each rh As System.Data.DataRow In th.Rows
            ...
            For Each rd As System.Data.DataRow In td.Rows
                ...
                If rc = 3 Then
                    ...
                    strDetail &= rd("PayTax").ToString() & "|"    '#12 จำนวนเงินภาษี
                    strDetail &= rd("PayTaxDesc").ToString() & "|"    '#13 ประเภทเงินได้
                    strDetail &= rd("PayTaxType").ToString() & "|"    '#14 เงื่อนไขการหักภาษี (1=หัก ณ ที่จ่าย, 2=ออกให้ตลอดไป, 3=ออกให้ครั้งเดียว 
                End If
            Next
            If rc = 1 Then
        ...
        If frm = "4" Then
            fname &= "3"
        End If
        fname &= "_" & ViewBag.PROFILE_TAXNUMBER & "_" & ViewBag.PROFILE_TAXBRANCH & "_" & DateTime.Now.Year() + 543 & "_" & DateTime.Now.Month.ToString("00")
