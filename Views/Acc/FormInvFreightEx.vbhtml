...
<table class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>:</td>
            <td><label id="quantity"></label> <label id="qtyUnit"></label></td>
        </tr>

        <tr>
            <td><label id="newBlNoLbl">M-B/L NO.</label></td>
            <td>:</td>
...
<tr>
            ...
            <td><label id="consLbl">B/L TYPE:</label></td>
            <td>:</td>
            <td><label id="blno"></label></td>
        </tr>
...
<table class="table" style="width:100%">
    ...
    <tr>
        ...
        <td class="textSpace" style="flex: 1; text-align: center;"><br /><br /><br /><br /><br /> _________________________________________</td>
        <td class="center" style="flex: 1; text-align: center;"> <br /><br /><br /><br /><br /><label id="EmpName">_________________________________________</label> </td>
    </tr>
    <tr>
        <td class="bold" style="text-align: center;">DATE : ________________________________</td>
        <td class="bold" style="text-align: center;">DATE : ________________________________</td>
        <td class="bold" style="text-align: center;">DATE : <label id="DocDate">________________________________</label> </td>
    </tr>
</table>
...
<script type="text/javascript">
    ...
    $.get(path + 'Acc/GetInvoice?Branch=' + branch + '&Code=' + code).done(function (r) {
        if (r.invoice.header.length > 0) {
            ...
            ShowUser(path, h.EmpCode, '#EmpName');
            $("#DocDate").text(ShowDate(h.DocDate));
  	 if (j.ShipBy == 1) {
		...
		$.get(path + 'Master/GetInterPort?Code=' + j.InvInterPort+ '&Key=' + j.InvCountry)
            ...
            $("#hblNo").text(j.HAWB);
            $("#quantity").text(j.InvProductQty);
            ShowInvUnit(path, j.InvProductUnit, '#qtyUnit');
            //$("#totpkg").text(j.TotalQty + " PALLETS");
            $("#newBlNo").text(j.MAWB +' / '+ j.BookingNo);
            ...
            for (let row of d) {
                ...
                if (row.AmtAdvance) {
                   // 	adv += ((row.Amt + row.AmtVat) * row.ExchangeRate.toFixed(4));
                    adv += ((row.AmtAdvance) * row.ExchangeRate.toFixed(4) + row.AmtVat).toFixed(2) - 0;
                } else {
                    if (row.AmtVat > 0) {
                        vat += (row.AmtCharge * row.ExchangeRate.toFixed(4)).toFixed(2)-0;
                        console.log((row.AmtCharge * row.ExchangeRate.toFixed(4)).toFixed(2));
            ...
            console.log(ShowNumber(varFromSrv, 2));
