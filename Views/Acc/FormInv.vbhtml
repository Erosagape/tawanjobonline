...
<table id="main" class="table" style="border-width:thin;border-collapse:collapse ;width:98%" >
    <thead>
        <tr class="upperLine underLine">
            ...
            <th class="center bold underLine" colspan="3" id="dvCurrency"></th>
        </tr>
        <tr class="upperLine">
...
<table class="table" style="width:98%;border-collapse:collapse ">
    ...
    <tbody>
	 ...
	 <tr>
            <td id="taxRate5"></td>
            <td id="gross5" class="center right"></td>
            <td id="wtAmt5" class="center right"></td>
...
*/
    $.get(path + 'Acc/GetInvoice?Branch=' + branch + '&Code=' + code).done(function (r) {
		...
		$("#crTerm").text(b.TermOfPayment);
                let docdate=ReverseDate(h.DocDate);		
		let termpayment=CNum(b.TermOfPayment);
	        
                //$("#dueDate").text(ShowDate(DateAdd(docdate,termpayment)));
		$("#dueDate").text(CDateEN(h.DueDate) !='' ? ShowDate(h.DueDate) : ShowDate(DateAdd(docdate,termpayment)));		
	        });
            $("#id").text(h.CustCode);
            ...
            $("#dvCurrency").html('AMOUNT IN '+ h.CurrencyCode);
	    ...
	    $("#hblNo2").text(j.HAWB);
            ...
            if (j.InvProductQty>0) $("#quantity").text(j.InvProductQty + ' ' + j.InvProductUnit);
	     $.get(path + 'Master/getcustomsunit?Code='+j.InvProductUnit).done(function (r) {
            ...
            if (j.TotalQty>0) $("#totpkg").text(j.TotalQty + " PACKAGE");
            $("#newBlNo").text(j.BookingNo);
            if (j.TotalGW>0)  $("#weight").text(ShowNumber(j.TotalGW, 2) + ' ' + j.GWUnit + '  '+'  ' + ' MEAS :' + j.Measurement + ')' );
	...
	$("#decl").text(j.DeclareNumber);
	    ...
	    let sumWht5 = 0;
            ...
            for (let row of d) {
                ...
                html += '            <td class="right">' + ShowNumber(row.AmtAdvance!==0 ? row.FUnitPrice + (row.AmtVat / row.Qty) : row.FUnitPrice, 2) + '</td>';
                html += '            <td class="right">' + (row.AmtAdvance!==0 ? ShowNumber(CNum(row.Amt) + CNum(row.AmtVat),  2):'') + '</td>';
                html += '            <td class="right">' + (row.AmtVat==0?(row.AmtCharge!==0?ShowNumber(row.Amt, 2):''):'') + '</td>';
                html += '            <td class="right">' + (row.AmtCharge!==0 &&row.AmtVat !==0?ShowNumber(row.Amt, 2) : '') + '</td>';
                html += '        </tr>';
                ...
                if (row.AmtAdvance!==0) {
                    ...
                    if (row.AmtVat !==0) {
                        vat += row.AmtCharge * row.ExchangeRate.toFixed(2);
                        varFromSrv += CNum(row.AmtVat,2);
                    } else {
                        nonVat += row.AmtCharge * row.ExchangeRate.toFixed(2);
                    }
                    switch (row.Rate50Tavi - 0) {
                        ...
                        case 3:
                            ...
                            sumbaseWht3 += row.Amt;
			     //console.log(row.Amt50Tavi+' : '+row.Amt);	
                            break;
			case 5:
			    sumWht5 += row.Amt50Tavi;
                            sumbaseWht5 += row.Amt;
                            break;
                        default:
                            break;
            ...
            $('#gross5').text(ShowNumber(sumbaseWht5, 2));
            $('#wtAmt5').text(ShowNumber(sumWht5, 2));
	     ...
	     $("#totalAmount2").text(ShowNumber(adv+vat +nonVat + h.TotalVAT, 2));
            let sumWht=sumWht1+sumWht3+sumWht1_5+sumWht5;
            //$("#lessWithholdingTax").text(ShowNumber(h.Total50Tavi, 2));
            $("#lessWithholdingTax").text(ShowNumber(sumWht, 2));
            $("#netAmount").text(ShowNumber(adv + vat + nonVat + h.TotalVAT - sumWht, 2));
            $("#custAdv").text(ShowNumber(h.TotalCustAdv, 2));
            $("#grandTotal").text(ShowNumber((adv + vat + nonVat + h.TotalVAT - sumWht)-h.TotalCustAdv, 2));

	    console.log((adv + vat + nonVat + h.TotalVAT - sumWht)-h.TotalCustAdv);
            
            $("#taxRate1").text("1%");
            $("#taxRate1_5").text("1.5%");
            $("#taxRate3").text("3%");
            $("#taxRate5").text("5%");
            let gt=(adv + vat + nonVat + h.TotalVAT - sumWht)-h.TotalCustAdv;
            $("#bahtText").text(h.CurrencyCode + ' ' + CNumEng(CDbl(gt,2)));
