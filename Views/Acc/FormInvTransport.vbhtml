...
<table class="table" style="width:100%">
    ...
    <tr>
        ...
        <td class="textSpace" style="flex: 1; text-align: center;"><br /><br /><br /><br /><br /> _________________________________________</td>
        <td class="center" style="flex: 1; text-align: center;"> <br /><br /><br /><br /><br /> <span style="text-decoration:underline;">@ViewBag.UserName</span></td>
    </tr>
    <tr>
    ...
    $.get(path + 'Acc/GetInvoice?Branch=' + branch + '&Code=' + code).done(function (r) {
		...
		$.get(path + 'Master/GetInterPort?Code=' + j.InvInterPort+ '&Key=' + j.InvCountry)
            ...
            for (let row of d) {
                ...
                html += '            <td class="right">' + (row.AmtAdvance==0?(row.AmtCharge?ShowNumber(row.Amt,2):''):'') + '</td>';
                //html += '            <td class="right">' + (row.AmtVat>0?ShowNumber(row.Amt,2) : '') + '</td>';
                html += '        </tr>';
                adv += row.AmtAdvance * row.ExchangeRate.toFixed(4);
		if (row.AmtCharge>0) { sumWht += row.Amt50Tavi; }
                if (row.AmtVat > 0) {
                    vat += row.AmtVat;
                    srv += row.AmtCharge * row.ExchangeRate.toFixed(4);
                } else {
            ...
            $('#sumWHTax').text(ShowNumber(sumWht,3));
            ...
            $("#sumSrv").text(ShowNumber(srv+nonVat,2));
            $("#sumVat").text(ShowNumber(vat,2));
            $("#totalNet").text(ShowNumber(srv+nonVat + vat ,2));
            $("#grandTotalNet").text(ShowNumber(srv+nonVat + vat - sumWht + adv,2));
            $("#totalPay").text(ShowNumber(srv+nonVat + vat - sumWht + adv,2));
            $("#bahtText").text(CNumThai(CDbl(srv+nonVat + vat - sumWht + adv,2)));
