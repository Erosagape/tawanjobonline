...
<div id="dvForm"  style="display:flex;flex-direction:column;">        
    ...
    <div style="border:1px solid black;border-radius:5px;margin-top:5px;padding:5px 5px 5px 5px;width:97%">
        ...
        <div style="display:flex">
            ...
            <div style="flex:1">
               SURRENDER DATE :<label id="lblSurrender"></label>
            </div>
            <div style="flex:1">
            </div>
        </div>
    ...
    <div style="width:97%;">
		...
		<td width="80px">EXC.</td>
            ...
            <tr>
                ...
                <td colspan="8">
                    <div style="text-align:left;"><label id="lblTotalBaht" style="font-size:13px;font-weight:bold"></label></div>
                </td>
            </tr>
            <tr>
                <td>TOTAL<br />(<label id="lblCurrency2"></label>)</td>
                <td colspan="8">
                    <div style="text-align:left;"><label id="lblTotalBaht2" style="font-size:13px;font-weight:bold"></label></div>
                </td>
            </tr>
        </table>
    </div>
<br/>
    <div style="display:flex;width:97%">
        <div class="text-left" style="border:1px solid black;border-radius:5px;flex:1;margin-right:5px;padding:5px 5px 5px 5px;font-size:10px;">
            ...
            <table style="width:100%;">
                ...
                <tr>
                    <td style="width:55%;font-size:10px;">RENT 5%</td>
                    <td style="width:25%;text-align:right;font-size:10px;"><label id="lblSumBaseWht5"></label></td>
                    <td style="width:20%;text-align:right;font-size:10px;"><label id="lblSumWht5"></label></td>
                </tr>
                <tr>
        ...
        <div style="border:1px solid black;border-radius:5px;flex:1;text-align:center;padding:5px 5px 5px 5px;font-size:10px;">
            FOR @ViewBag.PROFILE_COMPANY_NAME_EN <br /><br /> <br /><br /><br />
            ......................................................... <br />
            __________/_________/________ <br />
            AUTHORIZED SIGNATURE
        </div>
    ...
    @If ViewBag.DATABASE = "2" Then
        @<div style="border:1px solid black;border-radius:5px;margin-top:5px;padding:5px 5px 5px 5px;width:97%">
            Payable to : <span id="lblCompanyName2" style="font-weight:bold">Keopasit Co.,Ltd.</span> 
<br /> - Kasikorn Bank, Srinakarin Road Branch, Km.17, 
             <span id="lblAccountName2" style="font-weight:bold">Account Number 026-8-76046-6</span> <br /><br />
            Payment By <input type="checkbox" />Cash <input type="checkbox" />Transfer <input type="checkbox" /> Cheque No_______________/D_____________<br />
            Bank ___________________________________ Amount_______________________________Baht
        </div>
    Else
        @<div style="border:1px solid black;border-radius:5px;margin-top:5px;padding:5px 5px 5px 5px;width:97%">
           Payable to : <span id="lblCompanyName2" style="font-weight:bold">The Solution Logistics Co.,Ltd.</span>
            <br />- Kasikorn Bank, Srinakarin Road Branch, Km.17, <span id="lblAccountName2" style="font-weight:bold">Account Number 026-8-76862-9</span>
            <br />- Siam Commercial Bank, Ram Intra Road Branch (Fashion Island), <span id="lblAccountName3" style="font-weight:bold">Account Number 171-3-01198-0</span>
            <br />- Bangkok Bank, Theparak Road Branch, Km.22, <span id="lblAccountName4" style="font-weight:bold">Account Number 921-0-38888-1</span>
            <br />- Bangkok Bank, Theparak Road Branch, Km.22, <span id="lblAccountName5" style="font-weight:bold">Account Number 921-3-00552-4</span>
            <br />Payment By <input type="checkbox" />Cash <input type="checkbox" />Transfer <input type="checkbox" /> Cheque No_______________/D_____________<br />
             Bank ___________________________________ Amount______________________________Baht
        </div>
    End If
</div>
<script type="text/javascript">
    ...
    let ans = confirm('Show Tax-Invoice?');
    if (ans == true) {
        $('#lblDocType').html('ใบแจ้งหนี้ / ใบกำกับภาษี (INVOICE/TAX-INVOICE)');
    } 
    ...
    function ShowData(dr) {
	    ...
	    $('#lblCurrency2').text(h.CurrencyCode);
		...
		$('#lblDutyDate').text(ShowDate(CDateTH(j.DutyDate)));
		$('#lblSurrender').text(ShowDate(CDateTH(j.ImExDate)));
            }
            ...
            $('#lblSumNetInvoice').text(ShowNumber(CDbl($('#lblSumGrandTotal').text(), 2) - CDbl(Number(h.Total50Tavi),2) - CDbl(Number(h.TotalDiscount),2), 2));
            if (h.CurrencyCode == 'THB') {
                $('#lblTotalBaht').text('(' + CNumEng(CDbl(Number(h.TotalCharge)+Number(h.TotalAdvance)+Number(h.TotalVAT)-Number(h.Total50Tavi)-Number(h.TotalDiscount), 2)) + ')');
		$('#lblTotalBaht2').text('(' + CNumThai(CDbl(Number(h.TotalCharge)+Number(h.TotalAdvance)+Number(h.TotalVAT)-Number(h.Total50Tavi)-Number(h.TotalDiscount), 2)) + ')');
            } else {
                $('#lblTotalBaht').text('(' + CNumEng(CDbl(Number(h.TotalCharge)+Number(h.TotalAdvance)+Number(h.TotalVAT)-Number(h.Total50Tavi)-Number(h.TotalDiscount), 2)) + ')');
		$('#lblTotalBaht2').text('(' + CNumThai(CDbl(Number(h.TotalCharge)+Number(h.TotalAdvance)+Number(h.TotalVAT)-Number(h.Total50Tavi)-Number(h.TotalDiscount), 2)) + ')');
            }
        ...
        if (d.length > 0) {
            for (let o of d) {
                ...
                if (currCtn !== o.CTN_NO) {
                    currCtn = o.CTN_NO;
                    Html = '<tr>';
                    Html += '<td colspan="9">' + o.CTN_NO + ' / ' +  o.TRemark + '</td>';
                    Html += '</tr>';
                ...
                Html += '<td>' + o.SDescriptionEng + '</td>';
                Html += '<td style="text-align:center">' + o.CurrencyCode + '</td>';
                Html += '<td style="text-align:center">' + o.ExchangeRate + '</td>';
	...
	$('#lblSumWht5').text(ShowNumber(sumtax5, 2));
    }
