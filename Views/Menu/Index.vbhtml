...
select *
from (
select h.BranchCode,h.AdvNo,j.JNo,a.SDescription,j.CSCode,h.EmpCode,c.NameThai as CustomerName,h.JobType,h.ShipBy,h.PaymentDate as DocDate,
a.AdvNet as TotalAdvance,d.TotalClear
from Job_AdvHeader h inner join Job_AdvDetail a on h.BranchCode=a.BranchCode and h.AdvNo=a.AdvNo
left join Job_Order j on a.BranchCode=j.BranchCode and a.ForJNo=j.JNo
left join Mas_Company c on h.CustCode=c.CustCode and h.CustBranch=c.Branch
left join
(
select cd.BranchCode,cd.AdvNo,cd.AdvItemNo,sum(cd.BNet) as TotalClear
from Job_ClearDetail cd inner join Job_ClearHeader ch on cd.ClrNo=ch.ClrNo and ch.DocStatus<>99 and cd.AdvNO<>''
group by cd.BranchCode,cd.AdvNo,cd.AdvItemNo
) d
on a.AdvNo=d.AdvNO and a.ItemNo=d.AdvItemNo and a.BranchCode=d.BranchCode
where isnull(h.cancelprove,'')='' and isnull(h.PaymentRef,'')<>''
        ...
        If dt4.Rows.Count > 0 And dt5.Rows.Count > 0 Then
            ...
            @<div class="row">
                @if dt5.Rows.Count > 0 Then
		            ...
		            <a href="#tb4_1" class="btn btn-danger" onclick="SetDetail(7)"><b>ADV Unclear Details</b></a>
                        </div>
                    </div>
                End If
