...
@Code
    Dim accCode = Model("SICode")
    Dim docGroup = Model("DocGroup")
    Dim beginDate = ViewBag.BeginDate
    Dim endDate = ViewBag.EndDate
    Dim qry = ViewBag.QueryPL
    Dim sql = "
SELECT * FROM (
" & qry & "
) t
WHERE DocDate>='{0}' AND DocDate <='{1}' AND SICode='{2}' AND DocGroup='{3}'
"
    Dim tb = New CUtil(ViewBag.CONNECTION_JOB).GetTableFromSQL(String.Format(sql, beginDate, endDate, accCode, docGroup))
