...
<script type="text/javascript">
    ...
    function SetLOVs() {
        ...
        $.get(path + 'Config/ListValue?ID=tbX&Head=cpX&FLD=code,key,name', function (response) {
            ...
            //Branch
            CreateLOV(dv, '#frmSearchBranch', '#tbBranch', 'Branch', response, 4);
            CreateLOV(dv, '#frmSearchCust', '#tbCust', 'Customer', response, 2);
        });
    ...
    function SearchData(type) {
        switch (type) {
            case 'branch':
                ...
                break;
            case 'customer':
                SetGridCompany(path, '#tbCust', '#frmSearchCust', ReadCust);
                break;
        }
    }
    ...
    function ReadCust(dt) {
        $('#txtTaxNumber').val(dt.TaxNumber);
        $('#txtCustName').val(dt.NameEng);
    }
    function drawChart() {
