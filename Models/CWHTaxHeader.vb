...
Public Class CWHTaxHeader
    ...
    Public Function SaveData(pSQLWhere As String) As String
        ...
        Using cn As New SqlConnection(m_ConnStr)
            ...
            Catch ex As Exception
                Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CWHTaxHeader", "SaveData", ex.Message, True)
                msg = ex.Message
            End Try
        End Using
        Return msg
    ...
    Public Sub AddNew(pFormatSQL As String)
        If pFormatSQL = "" Then
            m_DocNo = ""
        Else
            Dim retStr As String = Main.GetMaxByMask(m_ConnStr, String.Format("SELECT MAX(DocNo) as t FROM Job_WHTax WHERE DocNo Like '%{0}' ", pFormatSQL), pFormatSQL)
            m_DocNo = retStr
        End If
    End Sub
    Public Function GetData(pSQLWhere As String) As List(Of CWHTaxHeader)
        ...
        Using cn As New SqlConnection(m_ConnStr)
            ...
            Try
                ...
                End While
            Catch ex As Exception
                Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CWHTaxHeader", "GetData", ex.Message, True, ex.StackTrace, "")
            End Try
        End Using
        Return lst
    End Function
        ...
        Using cn As New SqlConnection(m_ConnStr)
            Try
                ...
                Using cm As New SqlCommand("DELETE FROM Job_WHTax" + pSQLWhere, cn)
                    ...
                    cm.ExecuteNonQuery()
                    Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CWHTaxHeader", "DeleteData", cm.CommandText, False)
                End Using
                msg = "Delete Complete"
            Catch ex As Exception
                Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CWHTaxHeader", "DeleteData", ex.Message, True)
                msg = ex.Message
            End Try
        End Using
        Return msg
