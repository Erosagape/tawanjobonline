                ...
                Using da As New SqlDataAdapter("SELECT * FROM Mas_Company" & pSQLWhere, cn)
                    Using cb As New SqlCommandBuilder(da)
                        Using dt As New DataTable
                            ...
                            dr("CustGroup") = Main.GetDBString(Me.CustGroup, dt.Columns("CustGroup"))
                            ...
                            dr("CSCodeIM") = Main.GetDBString(Me.CSCodeIM, dt.Columns("CSCodeIM"))
                            dr("CSCodeEX") = Main.GetDBString(Me.CSCodeEX, dt.Columns("CSCodeEX"))
                            dr("CSCodeOT") = Main.GetDBString(Me.CSCodeOT, dt.Columns("CSCodeOT"))
                            dr("GLAccountCode") = Main.GetDBString(Me.GLAccountCode, dt.Columns("GLAccountCode"))
                            dr("CustType") = Main.GetDBString(Me.CustType, dt.Columns("CustType"))
                            dr("BillToCustCode") = Main.GetDBString(Me.BillToCustCode, dt.Columns("BillToCustCode"))
                            dr("BillToBranch") = Main.GetDBString(Me.BillToBranch, dt.Columns("BillToBranch"))
                            dr("UsedLanguage") = Main.GetDBString(Me.UsedLanguage, dt.Columns("UsedLanguage"))
                            dr("LevelGrade") = Main.GetDBString(Me.LevelGrade, dt.Columns("LevelGrade"))
                            dr("TermOfPayment") = Main.GetDBString(Me.TermOfPayment, dt.Columns("TermOfPayment"))
                            dr("BillCondition") = Main.GetDBString(Me.BillCondition, dt.Columns("BillCondition"))
                            dr("CreditLimit") = Me.CreditLimit
                            dr("MapText") = Main.GetDBString(Me.MapText, dt.Columns("MapText"))
                            dr("MapFileName") = Main.GetDBString(Me.MapFileName, dt.Columns("MapFileName"))
                            dr("CmpType") = Main.GetDBString(Me.CmpType, dt.Columns("CmpType"))
                            dr("CmpLevelExp") = Main.GetDBString(Me.CmpLevelExp, dt.Columns("CmpLevelExp"))
                            dr("CmpLevelImp") = Main.GetDBString(Me.CmpLevelImp, dt.Columns("CmpLevelImp"))
                            dr("Is19bis") = Me.Is19bis
                            dr("MgrSeq") = Me.MgrSeq
                            dr("LevelNoExp") = Me.LevelNoExp
                            dr("LevelNoImp") = Me.LevelNoImp
                            dr("LnNO") = Main.GetDBString(Me.LnNO, dt.Columns("LnNO"))
                            dr("AdjTaxCode") = Main.GetDBString(Me.AdjTaxCode, dt.Columns("AdjTaxCode"))
                            dr("BkAuthorNo") = Main.GetDBString(Me.BkAuthorNo, dt.Columns("BkAuthorNo"))
                            dr("BkAuthorCnn") = Main.GetDBString(Me.BkAuthorCnn, dt.Columns("BkAuthorCnn"))
                            dr("LtdPsWkName") = Main.GetDBString(Me.LtdPsWkName, dt.Columns("LtdPsWkName"))
                            dr("ConsStatus") = Main.GetDBString(Me.ConsStatus, dt.Columns("ConsStatus"))
                            dr("CommLevel") = Main.GetDBString(Me.CommLevel, dt.Columns("CommLevel"))
                            dr("DutyLimit") = Me.DutyLimit
                            dr("CommRate") = Me.CommRate
                            dr("TAddress") = Main.GetDBString(Me.TAddress, dt.Columns("TAddress"))
                            dr("TDistrict") = Main.GetDBString(Me.TDistrict, dt.Columns("TDistrict"))
                            dr("TSubProvince") = Main.GetDBString(Me.TSubProvince, dt.Columns("TSubProvince"))
                            dr("TProvince") = Main.GetDBString(Me.TProvince, dt.Columns("TProvince"))
                            dr("TPostCode") = Main.GetDBString(Me.TPostCode, dt.Columns("TPostCode"))
                            dr("DMailAddress") = Main.GetDBString(Me.DMailAddress, dt.Columns("DMailAddress"))
                            dr("PrivilegeOption") = Main.GetDBString(Me.PrivilegeOption, dt.Columns("PrivilegeOption"))
                            dr("GoldCardNO") = Main.GetDBString(Me.GoldCardNO, dt.Columns("GoldCardNO"))
                            ...
                            dr("ExportCode") = Main.GetDBString(Me.ExportCode, dt.Columns("ExportCode"))
                            dr("Code19BIS") = Main.GetDBString(Me.Code19BIS, dt.Columns("Code19BIS"))
                            dr("WEB_SITE") = Main.GetDBString(Me.WEB_SITE, dt.Columns("WEB_SITE"))
                            If dr.RowState = DataRowState.Detached Then dt.Rows.Add(dr)
                            da.Update(dt)
                            Main.SaveLogFromObject(My.MySettings.Default.LicenseTo.ToString, appName, "CCompany", "SaveData", Me, False)
                            msg = "Save Complete"
                        End Using
                    End Using
                End Using
            Catch ex As Exception
                Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CCompany", "SaveData", ex.Message, True, ex.StackTrace, "")
                msg = ex.Message
            End Try
        End Using
        Return msg
    ...
    Public Function GetData(pSQLWhere As String) As List(Of CCompany)
        ...
        Using cn As New SqlConnection(m_ConnStr)
            ...
            Try
                ...
                End While
            Catch ex As Exception
                Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CCompany", "GetData", ex.Message, True, ex.StackTrace, "")
            End Try
        End Using
        Return lst
    End Function
        ...
        Using cn As New SqlConnection(m_ConnStr)
            Try
                ...
                Using cm As New SqlCommand("DELETE FROM Mas_Company" + pSQLWhere, cn)
                    ...
                    cm.ExecuteNonQuery()
                    Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CCompany", "DeleteData", cm.CommandText, False)
                End Using
                msg = "Delete Complete"
            Catch ex As Exception
                msg = ex.Message
                Main.SaveLog(My.MySettings.Default.LicenseTo.ToString, appName, "CCompany", "DeleteData", ex.Message, True, ex.StackTrace, "")
            End Try
        End Using
        Return msg
    End Function
