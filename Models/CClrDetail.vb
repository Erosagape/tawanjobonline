                ...
                Using da As New SqlDataAdapter("SELECT * FROM Job_ClearDetail" & pSQLWhere, cn)
                    Using cb As New SqlCommandBuilder(da)
                        Using dt As New DataTable
                            ...
                            dr("Qty") = Me.Qty
                            dr("UnitCode") = Main.GetDBString(Me.UnitCode, dt.Columns("UnitCode"))
                            ...
                            dr("SlipNO") = Main.GetDBString(Me.SlipNO, dt.Columns("SlipNO"))
                            dr("Remark") = Main.GetDBString(Me.Remark, dt.Columns("Remark"))
                            dr("IsDuplicate") = Me.IsDuplicate
                            dr("IsLtdAdv50Tavi") = Me.IsLtdAdv50Tavi
                            dr("Pay50TaviTo") = Main.GetDBString(Me.Pay50TaviTo, dt.Columns("Pay50TaviTo"))
                            dr("NO50Tavi") = Main.GetDBString(Me.NO50Tavi, dt.Columns("NO50Tavi"))
                            dr("Date50Tavi") = Main.GetDBDate(Me.Date50Tavi)
                            dr("VenderBillingNo") = Main.GetDBString(Me.VenderBillingNo, dt.Columns("VenderBillingNo"))
                            dr("AirQtyStep") = Main.GetDBString(Me.AirQtyStep, dt.Columns("AirQtyStep"))
                            dr("StepSub") = Main.GetDBString(Me.StepSub, dt.Columns("StepSub"))
                            dr("JobNo") = Main.GetDBString(Me.JobNo, dt.Columns("JobNo"))
                            dr("AdvItemNo") = Me.AdvItemNo
                            dr("LinkBillNo") = Main.GetDBString(Me.LinkBillNo, dt.Columns("LinkBillNo"))
                            dr("VATType") = Main.GetDBString(Me.VATType, dt.Columns("VATType"))
